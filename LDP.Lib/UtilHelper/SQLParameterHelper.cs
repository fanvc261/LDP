...
namespace LDP.Lib.Data
{
    public class SqlParameterHelper
    {
        ...
        public DataTable ExecuteDataTable()
        {
            Debug.Assert((arParams.Length == index) && (paramCnt == index), "not all parameters were defined");
            if (debugLog) { log.Debug("ExecuteDataSet " + commandText); }
            DataSet ds = SqlHelper.ExecuteDataset(connectionString, cmdType, commandText, arParams);
        ...
        public DataTable ExecuteDataTable_ARITHABORT()
        {
            Debug.Assert((arParams.Length == index) && (paramCnt == index), "not all parameters were defined");
            if (debugLog) { log.Debug("ExecuteDataSet " + commandText); }
            DataSet ds = SqlHelper.ExecuteDataset_ARITHABORT(connectionString, cmdType, commandText, arParams);
            if (ds != null && ds.Tables.Count > 0)
                return ds.Tables[0];
            return new DataTable();
        }
