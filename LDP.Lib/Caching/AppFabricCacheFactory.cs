...
namespace LDP.Lib.Caching
{
    public class AppFabricCacheFactory: DistributedCacheFactoryBase
    {
        ...
        {
            string securityModeValue = ConfigHelper.GetStringProperty("Cache:AzureCacheSecurityMode", "Message").ToLowerInvariant();
            string securityAuthValue = ConfigHelper.GetStringProperty("Cache:AzureCacheAuthorizationInfo", string.Empty);
            bool useSsl = ConfigHelper.GetBoolProperty("Cache:AzureCacheUseSsl", true);
